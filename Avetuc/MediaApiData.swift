...
extension MediaApiData: Decodable {
    ...
    static func decode(j: JSON) -> Decoded<MediaApiData> {
        ...
        typealias Halved = Decoded<String -> String -> SizeApiData -> Int64 -> String -> Int64 -> String -> MediaApiData>

        let p: Halved = MediaApiData.create
            <^> j <| "id"
            <*> j <| "id_str"
            <*> j <|| "indices"
            <*> j <| "media_url"
